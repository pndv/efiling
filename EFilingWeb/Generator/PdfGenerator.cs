...
namespace EFilingWeb.Generator;

public class PdfGenerator {
    private readonly ILogger<PdfGenerator> logger;
    private readonly string xelatexPath;

    public PdfGenerator(ILogger<PdfGenerator> logger, string xelatexPath) {
        this.xelatexPath = xelatexPath;
        this.logger = logger;
    }
    ...
    public async Task<string> compileTex(string texFilePath, int compileCount, CancellationToken cancellationToken) {
        ...
        for (int i = 1; i <= compileCount; i++) {
            ...
            try {
                ...
                logger.LogError(e, "Error compiling file {FileName}. Message: {ExceptionMessage}", fileNameWithoutExtension, e.Message);
                throw;
            }
