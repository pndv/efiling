...
public class TexGenerator {
    ...
    public async Task<string> generateTeX(RetainerAgreementData data, CancellationToken cancellationToken) {
        ...
        logger.LogInformation("generateTex Start: case info {CaseInfo} for advocate {AdvocateName}", data.CaseDetails.getCaseInfo(), data.Advocate.GetFullName());
        ...
        await File.WriteAllTextAsync(outputFilePath, texCode);
